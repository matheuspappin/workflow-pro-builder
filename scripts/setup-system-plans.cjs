...
async function setupSystemPlans() {
  console.log('🚀 DanceFlow AI - Configurando system_plans...');
  ...
  try {
    ...
    console.log('📦 Tentando inserir planos...');
    ...
    if (error) {
      if (error.code === 'PGRST204' || error.code === 'PGRST205' || error.message.includes('not find the table')) {
        console.error('❌ Tabela "system_plans" não existe no banco de dados.');
        console.log('📝 DICA: Execute o arquivo database/system_plans.sql no SQL Editor do Supabase.');
      } else {
