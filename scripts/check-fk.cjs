...
async function checkForeignKeys() {
  ...
  console.log('Checking teachers relationship...');
  const { data: t, error: te } = await supabase.from('teachers').select('id, studio_id').limit(1);
  console.log('Teachers:', { data: t, error: te });

  console.log('Checking studios relationship...');
  const { data: s, error: se } = await supabase.from('studios').select('id').limit(1);
  console.log('Studios:', { data: s, error: se });

  console.log('Checking join teachers -> studios...');
  const { data: j, error: je } = await supabase.from('teachers').select('*, studios(id)').limit(1);
  console.log('Join Teachers -> Studios:', { data: j, error: je });

  console.log('Checking join users_internal -> studios...');
  ...
  console.log('Join Users Internal -> Studios:', { data: ui, error: uie });
}
