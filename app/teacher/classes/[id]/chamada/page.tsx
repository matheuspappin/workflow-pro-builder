...
export default function TeacherAttendancePage() {
  ...
  const loadAttendanceData = async () => {
    try {
      ...
      if (!session) {
        // Buscar o professor vinculado à turma
        ...
        const { data: newSession, error: createError } = await supabase
          ...
          .insert({
            ...
            actual_teacher_id: teacher?.id || null,
            status: 'agendada'
          })
          .select()
          .single()
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      for (const [studentId, status] of Object.entries(attendance)) {
        ...
        if (status !== oldStatus) {
          ...
          const message = isNowPresent 
            ? `Sua presença na aula de ${classInfo.name} do dia ${new Date().toLocaleDateString('pt-BR')} foi confirmada.`
            : `Foi registrada uma falta na aula de ${classInfo.name} do dia ${new Date().toLocaleDateString('pt-BR')}.`;
        ...
        if (isNowPresent && !wasPreviouslyPresent) {
          ...
          if (!usage) {
            ...
            if (credits && credits.remaining_credits > 0) {
              ...
              await supabase.from('student_credit_usage').insert({
                ...
                notes: `Presença em ${classInfo.name}`
              })
          ...
          if (credits) {
            ...
            await supabase.from('student_credit_usage').insert({
              ...
              credits_used: -1,
              usage_type: 'refund',
              notes: `Estorno de presença em ${classInfo.name}`
            })
          }
        }
      }
      ...
      toast({
        ...
        description: "As presenças foram salvas com sucesso."
      })
    } catch (error: any) {
      toast({
        title: "Erro",
    ...
    <main className="p-4 md:p-6 max-w-7xl mx-auto w-full space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight">Fazer Chamada</h2>
            <p className="text-xs text-muted-foreground">{classInfo?.name} • {new Date().toLocaleDateString('pt-BR')}</p>
          </div>
        </div>
        <Button 
          ...
          <CardHeader className="border-b border-slate-50 dark:border-slate-800">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-3 text-[10px] font-black uppercase">
                <span className="text-emerald-600">Sim: {Object.values(attendance).filter(v => v === 'present').length}</span>
                <span className="text-rose-600">Não: {Object.values(attendance).filter(v => v === 'absent').length}</span>
              </div>
            </div>
          </CardHeader>
                      ...
                      <div className="flex items-center gap-3 flex-1 overflow-hidden">
                        <Avatar className="h-10 w-10 border border-slate-100 shrink-0">
                          <AvatarFallback className={cn(
                            "font-bold text-xs",
                            attendance[student.id] === 'present' ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"
                          )}>
