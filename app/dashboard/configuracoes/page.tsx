...
function SettingsContent() {
  ...
  const verifyPayment = async (sessionId: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (userData) {
          const user = JSON.parse(userData)
          loadUsage(user.studio_id || user.studioId)
        }
      }
    } catch (error) {
  ...
  const getCurrentPlanLimits = () => {
    const plan = systemPlans.find(p => p.id === usage.plan)
    if (plan) return plan
    
    // Fallback para quando ainda não carregou ou não encontrou
    return {
      name: usage.plan,
      price: 0,
      max_students: 10,
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="plano" className="space-y-6">
            <Card className="bg-card border-border">
              ...
              <CardContent className="space-y-6">
                <div className={`p-6 border-2 rounded-xl ${usage.plan === 'gratuito' ? 'border-slate-200 bg-slate-50' : 'border-primary bg-primary/5'}`}>
                  <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
                    <div>
                      <h3 className="text-xl font-bold text-foreground uppercase tracking-tight">
                        {getCurrentPlanLimits().name}
                      </h3>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="text-right">
                        <p className="text-3xl font-bold text-primary">
                          R$ {Number(getCurrentPlanLimits().price || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-border/50">
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">{vocabulary.client}s Ativos</span>
                        <span className="font-bold">{usage.students} / {getCurrentPlanLimits().max_students >= 1000 ? 'Ilimitado' : getCurrentPlanLimits().max_students}</span>
                      </div>
                      <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-2 overflow-hidden">
                        <div 
                          className={`h-full transition-all ${usage.students >= (getCurrentPlanLimits().max_students || 0) ? 'bg-destructive' : 'bg-primary'}`}
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold">{usage.teachers} / {getCurrentPlanLimits().max_teachers >= 1000 ? 'Ilimitado' : getCurrentPlanLimits().max_teachers}</span>
                      </div>
                      <div className="w-full bg-slate-200 dark:bg-slate-800 rounded-full h-2 overflow-hidden">
                        <div 
                          className={`h-full transition-all ${usage.teachers >= (getCurrentPlanLimits().max_teachers || 0) ? 'bg-destructive' : 'bg-primary'}`}
                  ...
                  <div className="flex flex-wrap gap-3 mt-6">
                    <Badge variant={getCurrentPlanLimits().has_whatsapp ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().has_whatsapp ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      WhatsApp
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().has_ai ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().has_ai ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      Chat IA Full
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().has_finance ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().has_finance ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      Financeiro Avançado
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().hasPOS ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().hasPOS ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      PDV Local
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().hasInventory ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().hasInventory ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      Estoque
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().hasLeads ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().hasLeads ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      CRM / Leads
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().hasScanner ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().hasScanner ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      Scanner Entrada
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().hasGamification ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().hasGamification ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      Gamificação
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().hasMarketplace ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().hasMarketplace ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      Marketplace
                    </Badge>
                    <Badge variant={getCurrentPlanLimits().hasERP ? "default" : "secondary"} className="gap-1">
                      {getCurrentPlanLimits().hasERP ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      ERP Enterprise
                    </Badge>
